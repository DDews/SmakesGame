			...
			switch (messageType) {
				case DefaultMessageIDTypes.ID_NAT_PUNCHTHROUGH_SUCCEEDED:
					...
					if (!weAreTheSender) {
						...
						NetworkManager.guids.Add(rakPeer.GetMyGUID().g.ToString());
						NetworkManager.externalIPs.Add(rakPeer.GetInternalID().ToString(false));
						NetworkManager.externalPorts.Add(natListenPort);
